...
def data_split_remove(filename):
    ...
    # 保存数据集
    train_df.to_csv('../1-data/data_processed/assist09/train.csv', index=False)
    eval_df.to_csv('../1-data/data_processed/assist09/eval.csv', index=False)
    test_df.to_csv('../1-data/data_processed/assist09/test.csv', index=False)
...
if __name__ == '__main__':
    data_split_remove(filename='../1-data/data_raw/assist09/skill_builder_data_corrected.csv')

    train_dir = '../1-data/data_processed/assist09/train.csv'
    test_dir = '../1-data/data_processed/assist09/test.csv'

    processor = DatasetProcessor()
    inputs, target_id, target_correct, ids, correct, seq_len = processor.get_examples(train_dir)
    save_as_pt('../1-data/data_processed/assist09/train.pt', inputs, target_id, target_correct, ids, correct, seq_len)
    inputs, target_id, target_correct, ids, correct, seq_len = processor.get_examples(train_dir)
    save_as_pt('../1-data/data_processed/assist09/eval.pt', inputs, target_id, target_correct, ids, correct, seq_len)
